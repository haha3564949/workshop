...
def getWebData():
    ...
    counters = [Counter(name=scode,today=today) for scode in stock_info.index]
    ...
    # for scode in stock_info.index:
        ...
        # if scode.startswith('6'):
            ...
            # t = threading.Thread(target=get_Data(), args=(scode, today))
            # t.setDaemon(True)
            # # get_Data(scode,today)
            # t.start()
            # time.sleep(2)

def get_Data(scode,today):
